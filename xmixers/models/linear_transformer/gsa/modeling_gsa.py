...
class GsaLayer(nn.Module):
    def __init__(self, config: GsaConfig, layer_idx=0):
        super().__init__()
...
class GsaPreTrainedModel(PreTrainedModel):
    ...
    _no_split_modules = ["GsaLayer"]

    def _init_weights(self, module):
        return _init_weights(self, module)

    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, GsaModel):
...
class GsaForCausalLM(GsaPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        ```python
        >>> from transformers import AutoTokenizer, GsaForCausalLM

        >>> model = GsaForCausalLM.from_pretrained(PATH_TO_CONVERTED_WEIGHTS)
