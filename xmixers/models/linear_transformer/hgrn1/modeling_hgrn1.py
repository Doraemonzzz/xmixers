...
class Hgrn1Layer(nn.Module):
    def __init__(self, config: Hgrn1Config, layer_idx=0):
        super().__init__()
...
class Hgrn1PreTrainedModel(PreTrainedModel):
    config_class = Hgrn1Config
    supports_gradient_checkpointing = True
    _no_split_modules = ["Hgrn1Layer"]

    def _init_weights(self, module):
        return _init_weights(self, module)

    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, Hgrn1Model):
            module.gradient_checkpointing = value


class Hgrn1Model(Hgrn1PreTrainedModel):
    def __init__(self, config: Hgrn1Config):
        ...
        self.layers = nn.ModuleList(
            [Hgrn1Layer(config, layer_idx) for layer_idx in range(config.num_layers)]
        )
...
class Hgrn1ForCausalLM(Hgrn1PreTrainedModel, GenerationMixin):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.model = Hgrn1Model(config)
    ...
    def forward(
        ...
        **kwargs,
        ...
        ```python
        >>> from transformers import AutoTokenizer, Hgrn1ForCausalLM

        >>> model = Hgrn1ForCausalLM.from_pretrained(PATH_TO_CONVERTED_WEIGHTS)
