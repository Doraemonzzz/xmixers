# coding=utf-8
""" PyTorch ImplicitLinearTransformer model."""
...
class ImplicitLinearTransformerLayer(nn.Module):
    def __init__(self, config: ImplicitLinearTransformerConfig, layer_idx=0):
        super().__init__()
...
class ImplicitLinearTransformerPreTrainedModel(PreTrainedModel):
    config_class = ImplicitLinearTransformerConfig
    supports_gradient_checkpointing = True
    _no_split_modules = ["ImplicitLinearTransformerLayer"]

    def _init_weights(self, module):
        return _init_weights(self, module)

    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, ImplicitLinearTransformerModel):
            module.gradient_checkpointing = value


class ImplicitLinearTransformerModel(ImplicitLinearTransformerPreTrainedModel):
    def __init__(self, config: ImplicitLinearTransformerConfig):
        ...
        self.layers = nn.ModuleList(
            [
                ImplicitLinearTransformerLayer(config, layer_idx)
...
class ImplicitLinearTransformerForCausalLM(
    ImplicitLinearTransformerPreTrainedModel, GenerationMixin
):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.model = ImplicitLinearTransformerModel(config)
    ...
    def forward(
        ...
        **kwargs,
        ...
        ```python
        >>> from transformers import AutoTokenizer, ImplicitLinearTransformerForCausalLM

        >>> model = ImplicitLinearTransformerForCausalLM.from_pretrained(PATH_TO_CONVERTED_WEIGHTS)
