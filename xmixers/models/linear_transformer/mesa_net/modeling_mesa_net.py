...
class MesaNetLayer(nn.Module):
    def __init__(self, config: MesaNetConfig, layer_idx=0):
        super().__init__()
...
class MesaNetPreTrainedModel(PreTrainedModel):
    ...
    _no_split_modules = ["MesaNetLayer"]

    def _init_weights(self, module):
        return _init_weights(self, module)

    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, MesaNetModel):
...
class MesaNetModel(MesaNetPreTrainedModel):
    def __init__(self, config: MesaNetConfig):
        ...
        self.layers = nn.ModuleList(
            [MesaNetLayer(config, layer_idx) for layer_idx in range(config.num_layers)]
        )
...
class MesaNetForCausalLM(MesaNetPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs,
        ...
        ```python
        >>> from transformers import AutoTokenizer, MesaNetForCausalLM

        >>> model = MesaNetForCausalLM.from_pretrained(PATH_TO_CONVERTED_WEIGHTS)
