...
class TTTLayer(nn.Module):
    def __init__(self, config: TTTConfig, layer_idx=0):
        super().__init__()
...
class TTTPreTrainedModel(PreTrainedModel):
    ...
    _no_split_modules = ["TTTLayer"]

    def _init_weights(self, module):
        return _init_weights(self, module)

    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, TTTModel):
...
class TTTForCausalLM(TTTPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs,
        ...
        ```python
        >>> from transformers import AutoTokenizer, TTTForCausalLM

        >>> model = TTTForCausalLM.from_pretrained(PATH_TO_CONVERTED_WEIGHTS)
