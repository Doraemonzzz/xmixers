    ...
    def forward(
        ...
        **kwargs,
        ...
        k, v, k_head, v_head = map(
            lambda x: rearrange(x, "b n (d r) -> b n d r", r=self.kv_rank),
            [k, v, k_head, v_head],
        )

        k, v = map(
            lambda arr: torch.einsum("... d r, ... h r-> ... h d", arr[0], arr[1]),
            [(k, k_head), (v, v_head)],
        )
        q = rearrange(q, "... (h d) -> ... h d", h=self.num_heads)

        q, k, v = map(
            lambda x: rearrange(x.contiguous(), "... n h d -> ... h n d"),
            [q, k, v],
        )

        # apply l2 norm to q, k
